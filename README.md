...
- **汉化集成（含自动应用）**：自动检测 Freebuff 是否已应用[汉化包](https://github.com/Ximmmmmmm/freebuff-zh)（独立仓库）。Freebuff 更新把汉化覆盖回英文后，控制器会**自己换回中文**（默认开）——检测到装机版本变化、你点「启动」之前、以及刚拉到更新的汉化包时（装机是旧版汉化就直接升级）都自动应用，不必记得点按钮；仍然保留手动的「应用汉化 / 还原英文」，英文原版备份自动收敛到最近 2 份（旧的、写了一半的都清掉）
...
**自动应用汉化（默认开）**：控制器在三个时机自动换上 `hanhua/output/` 里的构建——
...
动手前逐条确认：开关打开、你没有手动选过英文、装机不是汉化版**或 `output/` 里的包比装机的更新**、
构建的 `targetVersion` 与装机版本一致、**且没有任何实例在运行**（换文件会打断任务，
有实例在跑就等下一个时机）。不满足就静默跳过，交回手动按钮，不弹任何对话框。
「还原英文」之后自动应用会暂停，直到你再点「应用汉化」——避免把你特意选回的英文又改回去。
开关状态存在 `%APPDATA%\FreebuffController\hanhua-auto.txt`（缺省开），窗口底部与托盘右键菜单里都能改；
...
（开关关着、你手动选过英文、有实例正在运行）才停在状态栏提示，等「应用汉化」或下一个时机。
