...
不满足就静默跳过，不弹任何对话框；但**因「有实例在跑」而被挡下的那次不会丢**：控制器记下
待办，每 10 秒再试一次（状态行会说一句「汉化待自动应用 · 关掉所有 Freebuff 实例后自动换上」），
所以关掉最后一个窗口后十秒内就会换上，不必去点任何东西。判定「有实例在跑」时只认
**主进程与各 slot 的顶层进程**：Electron 的子进程（`--type=renderer` / `gpu-process` / `utility`…）
不算——它们的 exe 路径与主进程完全一样，一个没退干净的子进程就能把自动应用永远卡住。
...
>
> **v1.8.23 修掉三个「自动应用静默不生效」的缝**，起因是一次实测：控制器起来时明明有可用构建、
> 装机也是英文，结果一直没换。
> ① 启动时那次恢复恒被同批的汉化包检查吞掉：`CheckPackUpdateAsync()` 一进来就把 `packBusy` 置 1
> （网络检查在后台跑），而自动应用看到 `packBusy` 就放行不接手——而它就在紧后面一行。现在启动
> 恢复挪到 pack 检查**之前**，冲突判定也从整段 `packBusy` 改成只在**暂存阶段**（真有文件写进
> `output/` 时）成立的 `packStaging`；反过来，暂存前会等正在换文件的那一轮读完，两边都不再可能
> 读到半份 `output/`。
> ② 因「有实例在跑」被拒时不再把构建指纹标成「已处理」就算了：临时原因留待办、10 秒一轮重试
> （`TryPendingHanhuaRestore`），构建完成时正用着 Freebuff 这个最常见的情形这才算兑现。
> ③ `QueryRunning` 不再把 Electron 子进程当实例（见上）。
