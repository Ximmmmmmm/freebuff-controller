...
- **多开**：主实例 + 实例 1~9，互不干扰
- **重复双击不再吃一句「已经被占用」**：控制器已经在跑时，再双击一次快捷方式，会把那个**已经存在的窗口直接叫到前台**（最小化 / 埋在窗口堆里都能叫回来），然后本实例安静退出。只有确实没人应答时才弹框（比如在跑的是旧版控制器，或那个进程卡死了），而且框里会写清**是哪个 PID、哪个完整路径、什么时候启动的**——照着去任务管理器里结束它即可（v1.8.24 起）。**v1.8.25 补了两处「以为生效了」**：① 抢前台是**有资格门槛**的（`SetForegroundWindow` 只对「收到最近一次输入」的进程放行，而那个进程是刚被双击启动的**新实例**），所以新实例先 `AllowSetForegroundWindow` 把资格让渡出去、再自己补一发提窗，主实例那一发才真跳到眼前——不给资格的话它只能闪一下任务栏，而 README 承诺的「埋在窗口堆里也能叫回来」恰好在这种情形下不成立；② 回执事件是 AutoReset 且没人排空，上一次超时后才落下的陈旧信号会让**下一次**双击一读就命中、误判「有人应答」于是安静退出（连兜底弹框都不弹），现在发请求前先清一次。另外窗口没真提到最前时不再无声无息：会弹一个气泡说明「窗口已还原，按 Alt+Tab 或点任务栏图标」
- **停止 = 先礼后兵（v1.8.25）**：「停止」/「停止全部」/「重置账号」先请实例自己退（`CloseMainWindow` 发 WM_CLOSE，最多等 2 秒）——硬杀不给 Chromium 与 profile 里那份 SQLite（`desktop-v2.db` 及其 `-wal`/`-shm`）收尾的机会；没退的才硬杀。杀完还会**顺着进程树把子孙一起收掉**：编排器 `bun-baseline.exe` 的 exe 名不是 `Freebuff.exe`，旧版既选不中它（筛选条件按名字查），`Process.Kill` 又不会连坐子进程，于是「停止」之后它一直挂着占端口/占地盘；「停止全部」还会顺手收掉上一次遗留的孤儿编排器（状态行报一句收掉了几个）。只碰装机目录里的进程，别的程序一律不认
- **失败日志（v1.8.25）**：这个工具通篇是「静默失败不拦流程」（不因边角异常崩掉），代价是出事时没有现场。关键路径的失败——换文件、杀进程、写各实例 `state.json`、写 `~\.AGENTS.md`、唤起窗口、建会话共享目录（`mklink /J`，v1.8.26 补）——现在会记一行到 `%TEMP%\freebuff-controller-log.txt`（文件超过 1 MB 自动清空重来）。不进界面、不打扰，只在下次「双击了没反应」「汉化没换上」时给出现场
- **多账号**：每个实例独立 Chromium 配置和登录态，可分别登录不同账号
- **状态一览**：实时显示每个实例的运行状态和当前登录的账号邮箱
- **额度显示**：每个账号的 **Freebucks** 额度（v0.0.88+ 起的新模型）——今日剩余 / 总额度，钱包有余额时附带显示；悬停可见剩余量、钱包余额、最便宜模型每小时单价与今日额度补充时间（太平洋每日 0 点）。请求会话接口时带 `x-freebuff-multi-session` / `x-freebuff-include-unused-rate-limits` 两个头（与桌面端 orchestrator 一致），否则服务器不返回 Freebucks。旧版服务端无 Freebucks 时回退为各模型中最紧的剩余额度。启动时与每 5 分钟自动更新（v1.9.0 起没有手动「刷新」按钮，全自动）。**没连代理就不查（v1.8.27）**：额度请求不再走控制器自己那两条兜底（系统代理 → 直连）——当前没有任何可用代理时（本地代理探测不到、系统代理也没配或不应答）**整轮跳过，一个请求都不发**，关了代理就不会有直连去碰服务端的请求；连上代理后自动恢复（跳过的那一轮不算「查过了」，不会白等 5 分钟），旧版手动「刷新」会提示跳过——该按钮已随 v1.9.0 精简下线。**跳过时额度列直接写「未连代理」**（v1.8.28，琥珀色，与真实数字区分开；上次读到过的值降级到悬停提示，旧数字不会再摆在那儿被当成当前额度，不刷新不等于归零）；判定「有没有代理」要真发探测（TCP + 功能级 204），所以它在后台线程跑，不会卡窗口。其余控制器请求（检查更新 / 下载汉化包 / 自更新）仍保留原来的候选链兜底
- **额度模型说明**：v0.0.88 起 Freebuff 不再有「每周 / 每月的会话上限」，旧的日/周/月窗口（`freeWindows`）已退役，本工具不再显示它们
- **版本检测**：显示本机 Freebuff 版本，自动对比官方更新源（**窗口里没有常驻的版本文字**——只有真有事时，按钮下方才浮出一行小字（8 秒后自动消失，平时完全空白）；发现新版时那行会浮出「有新版本 vX · 点此下载安装包」（可点击即下载，v1.9.x 起还会琺珀呼吸提醒；「检查更新」入口已随 v1.9.0 精简下线）；启动时与每 30 分钟的自动检查静默，但发现新版那行会浮出来提醒（同时弹一个气泡）；发现新版本可一键下载官方安装包——必须取到更新源的 `latest.yml`（文件名 + SHA512；缺任一项或校验值非法就直接中止，不运行没校验的包），下载到 `%TEMP%` 校验通过后立即运行，下载进度就显示在那一行（也同步到托盘悬停提示）、完成 / 失败另外弹气泡。**装完自动删包**：检测到装机版本追平下载时的目标版本后，把这份安装包删掉并在那一行报一句释放了多少（删不掉——安装器还占着文件——就留到下一轮检查再试）；官方 electron-updater 自己攒的缓存也在「装完」那一刻一起清（见「无用安装包清理」）
...
- **删除会话（v1.8.32，含全部聊天记录）**：Freebuff 本体的界面只有「归档 / 恢复会话」，没有删除入口；但本机 orchestrator 的本地 API 一直有删除能力——渲染进程封装里的 `deleteThread` → `POST /api/thread/<id>/delete`（UI 没接线），服务端删掉 threads 行后 `messages` / `queue_items` / `auto_run_decision_receipts` / `thread_deliveries` 全部 `ON DELETE CASCADE`，聊天记录随之物理删除。顶部一行「删除会话」→ 选会话库（共享库，或某实例未并入的独立库）→ 列出全部会话（标题 / 项目 / 最后活动 / 状态）→ 勾选 → 确认后永久删除，不可恢复。**写操作要 `x-freebuff-launch-id`**，而这个令牌由 Electron 主进程生成、只经同步 IPC 交给本窗口的渲染进程（从不进 argv，orchestrator 读完即从自己环境里抹掉），外部进程拿不到在跑实例的令牌——所以控制器自带令牌拉一个**临时 orchestrator** 来走这条 API：`FREEBUFF_DESKTOP_STATE_PATH` 指向临时 state（orchestrator 的跨进程锁是 per-state-path 的 SQLite 锁，fresh state 与真实例零冲突，**实例开着也能删**），临时目录里的 `projects` 建成指向真实例会话库的 junction（清理时**先非递归摘链接再删目录**，绝不递归穿进真库），launch id 自签自用、鉴权闭环自洽。删除走官方 `engine.deleteThread` 同一条代码路径，清理语义一致。草稿会话（没发过消息的）不在库里，删除时按「已不存在」跳过不算失败；删除后若对应标签页还开在某个 Freebuff 窗口里，把标签页关掉即可。已过 14 项冒烟（启动 / 列表 / 删除 / 消息级联 / 幂等 404 / junction 与临时目录清理）
- **挑要删的会话（v1.8.33）**：会话一多，「勾哪些」比「怎么删」更要紧，对话框（v1.9.1 精简后）只留四件东西——会话库下拉、搜索（匹配标题 / 项目名 / 项目路径，Esc 清空）、勾选列表（勾选框 / 标题 / 项目 / 最后活动 / 状态，固定最近活动降序）、四个按钮「全选 / 取消全选 / 删除选中 / 关闭」。勾选按会话 id 存、换搜索词不丢勾选；状态行给账「共 N 个 · 显示 M 个 · 已勾选 K 个」（勾了任何一条就转琥珀色）；**运行中的会话整行标琥珀色**，删除前确认框逐条列出清单并单独点名「其中 R 个正在运行」。防呆照旧：列表是真的「全部」（逐项目 open 合并，不受 `recentProjects` 8 条上限影响），项目打不开会写进状态行，全批 404 不报假成功。v1.9.1 起砍掉：项目 / 状态筛选下拉（搜索已覆盖）、点列头排序、键盘快捷键、刷新按钮
- **窗口内入口（不用碰托盘）**：顶部一行只有「删除会话 / 代理设置」（v1.9.0 精简后）——
  删除会话 = 列出所选会话库的全部会话，勾选后永久删除会话及其全部聊天记录（见上方「删除会话」）；
  代理设置 = 实时代理状态、逐端口探测、改地址 / 停用（off）/ 留空即自动探测。
  汉化、检查更新、清理安装包、额度刷新**全自动无入口**：有事才在按钮下方那行浮出一句话（发现新版那行可点击下载）。
  托盘右键菜单里只剩「退出」（双击托盘图标仍可把窗口叫回来）
- **会话共享（所有账号共用一份聊天记录，默认开启）**：不再有「共享会话」按钮——控制器启动时（或启动未共享的实例时）自动提示并入，确认后把每个实例的 `projects` 目录变成指向**主实例会话库**的 Windows junction（**v1.8.30：建链接改走原生 `DeviceIoControl(FSCTL_SET_REPARSE_POINT)`，`mklink /J` 只作兜底**——不靠拉 cmd.exe，因为实测装了安全软件的机器会把 `mklink` 一律拦成「拒绝访问」；两条路都失败时会把原生 Win32 错误码写进 `%TEMP%\freebuff-controller-log.txt`）——之后所有窗口读写同一个 `desktop-v2.db`，聊天记录天然只有一份：A 账号额度用完，B 账号的窗口打开同一份历史直接接着聊，**不需要复制、不需要选会话**。首次开启时各实例已有的独立历史会自动合并进主库（原目录改名保留为 `projects.pre-share-*` 备份，可手动回退），登录态仍在各自 state.json 下、账号互不影响。合并由 Freebuff 自带的 `resources\bun\bun.exe` 执行内嵌脚本完成（无外部依赖），同 id 会话绝不覆盖。提示：同一时间尽量只在一个窗口聊天——两个实例同时写入同一个库可能偶发锁冲突（WAL 模式下数据不会损坏）
- **两种初始化方式**：启动未初始化的实例时弹窗选择——全新登录（每个窗口用不同账号），或复制其他已登录实例的账号（下拉框只列出真正登录过的实例，并显示其邮箱），免重复登录。**v1.8.26 修了全新实例走不通的那条路**：启动顺序原本是「先建会话共享目录（junction）→ 才创建 `slots\slot-N`」，而 `mklink` 不会创建中间目录，父目录不存在就直接是「系统找不到指定的路径」——于是**每一个全新实例都卡在建 junction 失败**，而那次失败又被当成「还有实例没并入会话共享」处理，回头再调一次启动流程、再弹一次初始化弹窗（没有上限的递归：用户看到的就是「按了启动，还是要按启动」）。现在 ① 建 junction 前先补父目录，两个调用点都安全；② 真的建不起来时只弹一次框说清原因并中止，不再递归；③ 这次选的是「全新登录」还是「复制账号」会记在该实例目录里（`.controller-init`，随「重置账号」一起清掉），不再因为 Freebuff 还没写出 `state.json` 就每次启动都再问一遍
- **重置换号**：清空某个实例即可换登录另一个账号
- **汉化集成（全自动）**：自动检测 Freebuff 是否已应用[汉化包](https://github.com/Ximmmmmmm/freebuff-zh)（独立仓库）。Freebuff 更新把汉化覆盖回英文后，控制器会**自己换回中文**——检测到装机版本变化、你点「启动」之前、以及刚拉到更新的汉化包时（装机是旧版汉化就直接升级）都自动应用。**没有开关也没有按钮**：自动应用就是默认且始终生效的行为；英文原版备份自动收敛到最近 2 份（旧的、写了一半的都清掉）。**v1.8.25 起换文件不再有「半截」的可能**（先完整落地到 `ui.new-*`、校验 index.html 引用的哈希资源齐全、再两次改名换位，失败回滚——详见「汉化集成」），装机界面被判为不完整时会自动重装（v1.9.0 起「汉化状态」入口已下线——五个自动时机已覆盖所有场景，不再设手动兜底）
...
- **v1.9.x 白色简洁版（2026-09-24）**：① **纯白主题**——白底近黑字、浅灰面板/线条、蓝主色、绿/琥珀改白底可读的深色变体；标题栏不再跟随系统深色（显式指定 `DWMWA_CAPTION_COLOR` 纯白 + 近黑标题文字，Win10 自动回落浅色）；按钮文字按底色明度自选深浅。② **功能精简**——下线「刷新 / 检查更新 / 汉化状态」三个入口与常驻代理状态行（全自动覆盖，发现新版改走可点击状态行），代理设置对话框砍掉「恢复默认 / 停用」按钮（输入语义覆盖：留空 = 自动、off = 停用），删除会话对话框砍掉筛选 / 排序 / 快捷键（见「挑要删的会话」）。③ **动效**——窗口淡入、状态行淡入淡出、按钮悬停/按下渐变、表格行悬停高亮、发现新版琥珀呼吸、额度变化闪烁。④ **细节**——操作按钮防连点（点击后短暂禁用）、窗口位置记忆。⑤ **修真 bug**：`CreateJunctionNative` 重解析缓冲区错位 4 字节导致的 4392（v1.8.30 起一直坏，靠 `mklink` 兜底），修后自测 28/29 → **29/29 全过**，会话共享的原生建 junction 真正可用
- **浅色主题 UI**（v1.9.0 起；此前为暗色），单文件 exe（约 195 KB，无运行时依赖）；最小化进任务栏，关闭即完全退出
...
想确认当前状态：鼠标停在托盘图标上看悬停提示；换文件失败会弹气泡提醒（成功不打扰）。
窗口顶部那一行只有两个入口：**删除会话 / 代理设置**（v1.9.0 精简后；汉化状态 / 检查更新等全部转自动，托盘菜单只剩「退出」，双击托盘图标可把窗口叫回来）。
...
**换文件怎么换（v1.8.25 起是「先完整落地，再换位」）**：旧版是 `rm -rf ui` 紧接着逐文件拷
（与 `restore.sh` 同一套动作），而那会留下一个很宽的空窗：判定「汉化已应用」只看 `index.html`
里那行 `<html lang="zh-CN">`（哨兵），而 `index.html` 恰恰是拷贝的**第一个**文件（顶层文件先于
`assets/` 递归），于是哨兵在第一个毫秒就立住了、而实测 323 个文件 / 29 MB 资源还在路上；中途
任何一步失败（磁盘满 / 杀软锁文件 / 断电 / 控制器被结束）留下的就是「控制器认为已应用、实际
index.html 引用的哈希 bundle 不存在」的白屏态，且没有任何自动修复路径（备份只在「未应用」时
才留，也没有完整性校验）。现在：先整份拷到同卷的 `ui.new-<时间戳>` → 校验 `index.html` 引用的每个
`./assets/…` 都在（源自己就是半截的就直接拒绝，装机分毫不动）→ `Directory.Move` 两次换位
（同级改名是元数据操作，毫秒级）→ 删旧份。于是装机目录要么是旧的完整份、要么是新的完整份；
第二次改名失败会把旧的移回来，回滚也失败时留着 `ui.old-*` 不删（它是唯一的完整旧份），
启动时顺手清理这些中间目录。另外，「汉化已应用但界面不完整」现在会被认出来（状态行写
「汉化 ✗ 界面不完整 · 待重装」并按琥珀色显示，日志也记一行），自动重装不再需要「有新包」
这个前提（v1.9.0 起「汉化状态」入口已下线，强制重装随之取消——自动重装已覆盖「不完整」场景）。
...
离线自测：`FreebuffController.exe --self-test %TEMP%\selftest.txt`（退出码 0 = 全过）——
把「换文件」这条最容易出事、又最难手工复现的路径在临时目录里的假树上跑一遍：完整性校验认得出
好/坏两份、正常换位后新内容到位且不留中间目录、**源是半截时必须拒绝且装机那份原样不动**、
中间目录清理的例外规则（装机 ui 不在时 `ui.old-*` 必须留着）、原子写不留临时文件、
进程树查找只认源自目标进程的子孙。不碰装机目录、不开窗口，也不会往失败日志里写东西
（自测会故意造畸形夹具，那些预期中的报错只进报告），CI 的 build 流程也会跑它。
...
> 源码需兼容 C# 5（系统自带编译器的语言版本：不能写 `?.`、`$""`、`nameof`、表达式体成员）。

编译产物就是正在跑的那个 exe 自己，所以**控制器开着时写不进去**（报 `CS0016: 未能写入输出文件`）：
先退出控制器（托盘右键 → 退出）再编译。编译后用自测确认换文件那套逻辑没被改坏：
